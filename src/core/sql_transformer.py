...
class SQLTransformer:
    ...
    def rewrite_structural(sql: str) -> str:
        ...
        sql = SQLTransformer._rewrite_aces_aggregation(sql)
    ...
    # ============================================================
    # Rewrite generic aces aggregation → deterministic CASE logic
    # ============================================================

    @staticmethod
    def _rewrite_aces_aggregation(sql: str) -> str:
        """
        Detect naive SUM(m.aces) patterns generated by the LLM
        and rewrite them into deterministic winner/loser CASE logic
        using w_ace and l_ace columns.
        """

        lowered = sql.lower()

        # Detect invalid "m.aces" or generic "aces" aggregation
        if "m.aces" not in lowered and "sum(aces" not in lowered:
            return sql

        # Replace SUM(m.aces) or SUM(aces)
